...
test.describe('menu', () => {
    regressionTest('basic md - no default icon on second level, scaling for popup window', async ({ page }) => {
    ...
    regressionTest('basic lg - no default icon on second level, visible ellipsis for category', async ({ page }) => {
    ...
    regressionTest('link - should render menu items with a[href]', async ({ page }) => {
        await page.setViewportSize(viewPorts.lg);
    ...
    regressionTest('category open on expand when initially closed and activated', async ({ page }) => {
        await page.setViewportSize(viewPorts.lg);
    ...
    regressionTest('show scrollbar on overflow on expand on lg', async ({ page }) => {
        await page.setViewportSize(viewPorts.lg);
    ...
    regressionTest('show scrollbar on overflow on expand on md', async ({ page }) => {
        await page.setViewportSize(viewPorts.md);
        await page.goto('menu/overflow');
    ...
    regressionTest('show scrollbar on overflow on expand on sm', async ({ page }) => {
    ...
    regressionTest('hide scrollbar on collapsed but scrollable', async ({ page }) => {
        await page.setViewportSize(viewPorts.lg);
        await page.goto('menu/overflow');
        ...
        await collapseButton.click();
        await page.waitForTimeout(1000);
        expect(await application.screenshot({
            animations: 'disabled',
        })).toMatchSnapshot();
    });
});
